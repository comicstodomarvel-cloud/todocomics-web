...
export default function OnlineCounter({ variant = 'default' }: { variant?: 'default' | 'toolbar' }) {
  ...
  if (variant === 'toolbar') {
    return (
      <div className="group relative flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-zinc-900/60 border border-zinc-800 text-xs shrink-0">
        <span className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse" />
        <Users size={12} className="text-zinc-500" />
        <span className="font-medium text-zinc-300">{online}</span>
        <span className="text-zinc-500 hidden sm:inline">
          {online === 1 ? 'usuario' : 'usuarios'}
        </span>
